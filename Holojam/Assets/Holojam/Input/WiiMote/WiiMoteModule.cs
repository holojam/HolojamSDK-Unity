...
namespace Holojam.IO {
	...
	public class WiiMoteModule : MonoBehaviour {
		...
		Dictionary<int, List<GlobalReceiver>> buttonReceivers = new Dictionary<int, List<GlobalReceiver>>();

		void Awake() {
			//initialize button pairings and global objects
			foreach (int button in buttons) {
				buttonPairings.Add(button, null);
				buttonReceivers.Add(button, new List<GlobalReceiver>());
			}
		}
		...
		void CastRayFromBoundObject() {
			...
			if (hits.Count == 0) {
				...
				eventData.raycastHit = new RaycastHit();
				return;
			}
		...
		#region ExecuteGlobal Functions
		...
		void OnDrawGizmos() {
			...
			if (eventData != null && eventData.currentRaycast != null) {
				Gizmos.DrawLine(this.transform.position, eventData.raycastHit.point);
			} else {
